...
if (CustomerType == 1)
{
	if (QWaterConsumption <= 5)
	{
		...
		QAmountToPay = QFreshWaterCharge + QStandingCharges + QSurfaceWaterCharge + QWasteWaterCharge + QVAT;
		QTotalDomestic += QWaterConsumption;
		QTotalFreshWater += QWaterConsumption;
		QRevenue += QAmountToPay;
		count += 1;
		QTotalDomestic += QWaterConsumption;
		QTotalFreshWater += QWaterConsumption;
		QCost += QWaterConsumption;
		totalbill += QAmountToPay;
		if (QTotalCharge > QMaxDomesticBill)
		{
			QMaxDomesticBill = QTotalCharge;
		}
	}
	if (QWaterConsumption>5 && QWaterConsumption <= 12)
	{
	...
	if (QWaterConsumption>12 && QWaterConsumption <= 25)
	{
		QFreshWaterCharge = (QWaterConsumption-12) * 0.50 + 5 *0.20 + 7 * 0.35;
		QStandingCharges = 0.10 * 91;
		QSurfaceWaterCharge = 10;
		QWasteWaterCharge = 0.95 * QWaterConsumption * 0.25;
		QVAT = 0;
	...
	if (QWaterConsumption > 25 && QWaterConsumption <= 40)
	{
		QFreshWaterCharge = (QWaterConsumption-25) * 0.75 + 5 *0.20 + 7 * 0.35 + 13 * 0.5;
		QStandingCharges = 0.10 * 91;
		QSurfaceWaterCharge = 10;
		QWasteWaterCharge = 0.95 * QWaterConsumption * 0.25;
		QVAT = 0;
	...
	{
		QFreshWaterCharge = (QWaterConsumption-40) * 2.5 + 5 *0.20 + 7 * 0.35 + 13 * 0.5 + 15 * 0.75;
		QStandingCharges = 0.10 * 91;
		QSurfaceWaterCharge = 10;
		QWasteWaterCharge = 0.95 * QWaterConsumption * 0.25;
		QVAT = 0;
...
if (CustomerType == 2)
{
	...
	QWasteWaterCharge = 0.95 * QWaterConsumption * 2;
	QTotalCharge = QFreshWaterCharge + QStandingCharges + QSurfaceWaterCharge + QWasteWaterCharge;
...
QAverageDomesticBill = totalbill/count;
